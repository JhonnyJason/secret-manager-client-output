...
//###########################################################
//region imports
var acceptSecretsFrom, addNotificationHook, authenticateResponse, createAuthCode, createSignature, decrypt, deleteNotificationHook, deleteSecret, deleteSecretSpace, deleteSharedSecret, encrypt, getNotificationHooks, getSecret, getSecretFrom, getSecretSpace, getSignedNodeId, getSubSpace, openSecretSpace, prepareNewKeys, setSecret, shareSecretTo, stopAcceptingSecretsFrom;
...
export var Client = class Client {
  ...
  async generateAuthCodeFor(action) {
    await this.ready;
    return (await createAuthCode(action, this));
  }

  async getServerId(authCode) {
    var content, idHex, response, sigHex, timestamp;
    await this.ready;
    response = (await getSignedNodeId(authCode, this));
    // {
    //     "serverNodeId": "...",
    //     "timestamp": "...",
    //     "signature": "..."
  ...
  //#######################################################
  async shareSecretTo(shareToId, secretId, secret, isOneTimeSecret) {
    await this.ready;
    if (shareToId instanceof Client) {
      shareToId = shareToId.id;
    }
    secret = (await encrypt(secret, shareToId));
    return (await shareSecretTo(shareToId, secretId, secret, isOneTimeSecret, this));
  }
  ...
  //#######################################################
  incNonce() {
    this.nonce = ++this.nonce % 10000000;
  }
...
//###########################################################
authenticateResponse = async function(content, sigHex, idHex, timestamp) {
  var err, verified;
  try {
    if (!timestamp) {
    ...
    err = error;
    throw new Error("Error on authenticateResponse! " + err);
  }
};
...
//###########################################################
//region effectiveSCI
getSignedNodeId = async function(authCode, client) {
  var reply, server;
  server = client.serverURL;
  reply = (await sci.getNodeId(server, authCode));
  if (reply.error != null) {
    throw new Error("getSignedNodeId replied with error: " + reply.error);
  }
  return reply;
};

//###########################################################
createAuthCode = async function(action, client) {
...
openSecretSpace = async function(client, authCode) {
  ...
  console.log(`openSecretSpace - ${authCode}`);
...
shareSecretTo = async function(shareToId, secretId, secret, isOneTimeSecret, client) {
  var nonce, payload, publicKey, reply, route, secretKey, server, signature, timestamp;
  server = client.serverURL;
  publicKey = client.publicKeyHex;
  secretKey = client.secretKeyHex;
  timestamp = validatableStamp.create();
  if (isOneTimeSecret == null) {
    isOneTimeSecret = false;
  }
  nonce = client.nonce;
  client.incNonce();
  payload = {publicKey, shareToId, secretId, secret, isOneTimeSecret, timestamp, nonce};
  route = "/shareSecretTo";
  signature = (await createSignature(payload, route, secretKey));
  reply = (await sci.shareSecretTo(server, publicKey, shareToId, secretId, secret, isOneTimeSecret, timestamp, signature, nonce));
  if (reply.error != null) {
...
// ############################################################
// setRequestableServer = (serverURL, serverNodeId, authCode, client) ->
//     server = client.serverURL
//     reply = await sci.setRequestableServer(server, authCode, serverURL, serverNodeId)
//     if reply.error? then throw new Error("setRequestableServer replied with error: "+reply.error)
//     return reply
